...
uint256 private unlockFarmPrice;
 ...
 function getPoolBalance() public view returns(uint256){
  ...
  function _updateFarm(address user, uint256 totalMint, uint256 nextWateringTime, bool status, uint256 endTime) internal {
     farms[user].totalMint = farms[user].totalMint += totalMint;
     farms[user].nextWateringTime = nextWateringTime;
     farms[user].status = status;
     farms[user].endTime = endTime;
  }
